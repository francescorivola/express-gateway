...
// Helper function to convert Express 4 path syntax to Express 5
function normalizePathForExpress5(path) {
  if (typeof path !== "string") {
    return path; // Return as-is for RegExp or other types
  }

  // Convert bare wildcard "*" to named wildcard "/*splat" (matches any non-root path)
  if (path === "*") {
    return "/*splat";
  }

  // Convert trailing /* to /{*splat} to match zero or more segments
  // This allows /admin/* to match both /admin/ and /admin/foo
  path = path.replace(/\/\*$/, "/{*splat}");

  // Convert /path/*/other/* patterns - replace each remaining /* with /*splat{number}
  let counter = 0;
  path = path.replace(/\/\*/g, () => {
    counter++;
    return `/*splat${counter}`;
  });
  ...
  for (const el in apiEndpoints) {
    ...
    for (const route of hostConfig.routes) {
      ...
      for (let path of mountPaths) {
        // Normalize path for Express 5 compatibility
        path = normalizePathForExpress5(path);
