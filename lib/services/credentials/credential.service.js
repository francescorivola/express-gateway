...
const generateKey = customAlphabet(alphanumeric, 22);
const s = {};
    ...
    if (areMultipleCredsAllowed) {
      return Promise.all([
        ...
        .then(([scopes, credentialProps]) => {
          ...
          newCredential.keyId = credentialDetails.keyId || generateKey();
          newCredential.keySecret =
            credentialDetails.keySecret || generateKey();
...
function validateNewCredentialScopes(credentialConfig, credentialDetails) {
  ...
  if (
    ...
    credentialConfig.required.includes("scopes")
  ) {
