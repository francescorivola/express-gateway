...
s.insert = function (user) {
  return validateAndCreateUser(user).then(function (newUser) {
    return userDao.insert(newUser).then(function (success) {
      if (success) {
...
s.get = function (userId, options) {
  ...
  return userDao.getUserById(userId).then(function (user) {
    if (!user) {
      return false;
    }
