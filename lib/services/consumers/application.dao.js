...
dao.update = function (id, props) {
  ...
  return db.hmset(hashKey, props).then(function (res) {
    return !!res;
  });
};
...
dao.find = function (appName) {
  return db.smembers(appNameSetKey(appName)).then(function (Ids) {
    if (Ids && Ids.length !== 0) {
      if (Ids.length === 1) {
        return Ids[0];
      }
...
dao.get = function (id) {
  return db.hgetall(appHashKey(id)).then(function (app) {
