...
describe("User service tests", () => {
  describe("Insert tests", () => {
    ...
    it("should insert a user", () => {
      ...
      return userService.insert(user).then(function (newUser) {
  ...
  describe("Get and Find User tests", () => {
    ...
    it("should get user by userId", () => {
      return userService.get(user.id).then(function (_user) {
        const expectedUserProps = [
          "firstname",
          "lastname",
          "email",
    ...
    it("should not get user by invalid userId", () => {
      return userService.get(randomUUID()).then(function (user) {
        should.exist(user);
    ...
    it("should find user by username", () => {
      return userService.find(user.username).then(function (_user) {
        const expectedUserProps = [
          "firstname",
          "lastname",
          "email",
    ...
    it("should not find user by invalid username", () => {
      return userService.find("invalid_username").then(function (user) {
        should.exist(user);
        user.should.eql(false);
      });
    });
  ...
  describe("Update user tests", () => {
    ...
    it("should update user", () => {
      ...
      return userService.update(user.id, updatedUser).then(function (res) {
        should.exist(res);
        res.should.eql(true);
        return userService.get(user.id).then(function (_user) {
    ...
    it("should allow update of any single user property user", function () {
      ...
      return userService
        .update(user.id, anotherUpdatedUser)
        .then(function (res) {
          res.should.eql(true);
          return userService.get(user.id).then(function (_user) {
    ...
    it("should not update user with unvalid id", function () {
      ...
      return userService.update("invalid_id", updatedUser).then(function (res) {
        should.exist(res);
  ...
  describe("Activate and deactivate user tests", () => {
    ...
    it("should deactivate user", function (done) {
      userService.deactivate(user.id).then(function (res) {
        should.exist(res);
        res.should.eql(true);
        return userService
          .get(user.id)
    ...
    it("should reactivate user", function (done) {
      userService.activate(user.id).then(function (res) {
        res.should.eql(true);
        return userService
          .get(user.id)
          .then(function (_user) {
  ...
  describe("Delete user tests", () => {
    ...
    it("should delete user", () => {
      return userService.remove(user.id).then(function (deleted) {
    ...
    it("should not delete user with invalid id", () => {
      return userService.remove("invalid_id").then(function (deleted) {
        should.exist(deleted);
    ...
    describe("should delete all the related credentials", () => {
      ...
      before(() =>
        Promise.all([
          ...
          (
            [_, jwt1, jwt2], // eslint-disable-line no-unused-vars
          ) =>
            Promise.all(
              [jwt1, jwt2].map((cred) => {
